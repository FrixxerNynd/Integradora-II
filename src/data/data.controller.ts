...
export class DataController {
    ...
    @Get('all')
    async getAllData() {
        const res$ = this.httpService.get('http://localhost:3001/api/data/all');
    ...
    async getSearchData(@Query() queryParams: any) {
        const res$ = this.httpService.get('http://localhost:3001/api/data/search', {
            params: queryParams,
        });
        const res = await lastValueFrom(res$);
        return res.data;
    ...
    async deleteData(@Param('id') id: string) {
        const res$ = this.httpService.delete(`http://localhost:3001/api/data/delete/${id}`);
        const res = await lastValueFrom(res$);
        return res.data;
    }
}